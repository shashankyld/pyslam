            ...
            if not is_paused or do_step:
                ...
                if img is not None:
                    ...
                    if img is not None:
                        ...
                        ###############MAPSNAPSHOT - STEP 1
                        ref_map_snapshot = slam.map.snapshot_manager.get_snapshot(Parameters.kCompareMapSnapshotIndex)
                        if ref_map_snapshot is not None:
                            ref_map_coordinates = ref_map_snapshot.coordinates
                            log_snapshot_map(index=Parameters.kCompareMapSnapshotIndex, entity_path="world/slam", points=ref_map_coordinates)
