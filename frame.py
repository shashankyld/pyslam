...
class Frame(FrameBase):
    ...
    def __init__(self, img, camera, pose=None, id=None, timestamp=None, kps_data=None):
        ...
        self.octaves = None      # keypoint octaves                      [Nx1] # q: What are keypoint octaves? a: The octave is the level of the scale pyramid where the keypoint has been detected
        ...
        if img is not None:
            ...
            if kps_data is None:   
                ...
                print("Descriptor shape: ", self.des.shape)
    ...
    def apply_mask(self, mask):
        ''' 
        Mask is generated by a network as an image with values in {0,1}
        '''
        # Resize the mask to match the image dimensions
        mask = cv2.resize(mask, (self.width, self.height))

        # Show the mask and the image with the mask applied
        cv2.imshow('mask', mask)
        cv2.waitKey(100)
        
        print("Image shape: ", self.img.shape)
        print("kps shape: ", self.kps.shape)
        print("kps", self.kps)
        
        # Create a mask to filter out keypoints
        keep_indices = []
        for i in range(len(self.kps)):
            x = int(self.kps[i][0])
            y = int(self.kps[i][1])
            if mask[y, x] == 1:  # Check the mask value
                keep_indices.append(i)

        # Use the indices to filter out keypoints and associated data
        self.kps = self.kps[keep_indices]
        self.kpsu = self.kpsu[keep_indices]
        self.kpsn = self.kpsn[keep_indices]
        self.octaves = self.octaves[keep_indices]
        self.sizes = self.sizes[keep_indices]
        self.angles = self.angles[keep_indices]
        self.des = self.des[keep_indices]
