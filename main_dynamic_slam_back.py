...
from utils_delaunay import delaunay_visualization, filter_delaunay_edges_by_3d_distance
...
from utils_delaunay import draw_simplicies_on_image, convert_frame_to_kdtree, get_common_edges, convert_frame_to_reference_dict, convert_frame_to_kdtree_masked
...
from utils_misc import remove_duplicates_from_index_arrays
    ...
    while True: 
        ...
        if img is not None:
            ...
            if img_id > starting_img_id:
                ...
                '''
                idxs_ref, idxs_cur, num_found_map_pts = search_frame_by_projection(prev_frame, cur_frame, 
                                                                             max_reproj_distance= Parameters.kMaxReprojectionDistanceFrame  ,
                                                                             max_descriptor_distance=slam.tracking.descriptor_distance_sigma,
                ...
                # for i in range(len(idxs_cur)):
                #     print(idxs_cur[i], idxs_ref[i])


                # Add validation check here
                if len(idxs_cur) < 3:  # Minimum points for Delaunay
                    print("Warning: Not enough matched features (%d), skipping Delaunay" % len(idxs_cur))
                    # prev_dict = {}
                    # continue
                ...
                # curr_dict = convert_frame_to_reference_dict(slam, idxs_cur)
                curr_dict = convert_frame_to_kdtree_masked(slam, idxs_cur)
                print("Current dict - created: ", len(curr_dict))
                ...
                if Parameters.kShowDebugImages:
                    visualize_common_simplicies(curr_img, prev_img, curr_dict, prev_dict)
                ...
                @ LEGACY CODE   
                ## FOLOWWING CODE IS FOR TESTING THE COMMON EDGES FUNCTION 
                kpsu_ref = prev_frame.kpsu 
                kpsu_cur = cur_frame.kpsu
                print("Number of keypoints in the previous frame: ", len(kpsu_ref))
                print("Number of keypoints in the current frame: ", len(kpsu_cur))
                # Filter with idxs_ref and idxs_cur 
                kpsu_ref = [kpsu_ref[i] for i in idxs_ref]
                kpsu_cur = [kpsu_cur[i] for i in idxs_cur]
                print("Number of keypoints in the previous frame after filtering: ", len(kpsu_ref))
                print("Number of keypoints in the current frame after filtering: ", len(kpsu_cur))
                # print("kpsu_ref: ", kpsu_ref)
                # Print dict
                # print("Prev dict: ", prev_dict)
                # Remove duplicates from the kpsu_ref, kpsu_cur
                
                # Convert to tuples of integers, assuming kpsu_ref[i] is a list/tuple of two elements
                kpsu_ref = [(int(kp[0]), int(kp[1])) for kp in kpsu_ref]
                kpsu_cur = [(int(kp[0]), int(kp[1])) for kp in kpsu_cur]

                # Remove duplicates (if necessary - you might want to rethink if duplicate removal is still needed here)
                kpsu_ref = list(set(kpsu_ref))
                kpsu_cur = list(set(kpsu_cur))

                print("Number of keypoints in the previous frame after removing duplicates: ", len(kpsu_ref))
                print("Number of keypoints in the current frame after removing duplicates: ", len(kpsu_cur))

                '''
                
                # print("Prev dict: ", img_id, len(prev_dict))
...
################### 
# APPLY DELAUNAY IN THE COMMON POINTS ALONE
# I MISTAKENLY DID IT FOR THE WHOLE KPS IN EACH FRAME AND 
# CHECKED FOR COMMON EDGES BASED ON DELAUNAY TRIANGULATION OF THE WHOLE KPS IN THE CURRENT FRAME
# EVEN IDXS_CURR AND IDXS_REF HAVE DUPLICATE POINTS
# ONE BLUNDER IS THAT I STARTED USING KPSU INT for indexing, which i should not have done - try to fix it. for now, trying to remove duplicates from the kpsu_common
###################