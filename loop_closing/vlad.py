...
class VLAD:
    ...
    def place_recognition_query(self, database_descs, query_descs):
        ...
        similarities = [torch.cosine_similarity(query_vlad, db_vlad, dim=0).item() for db_vlad in database_vlads]
        
        # Sort indices by similarity to get the most similar places
        most_similar_idx = sorted(range(len(similarities)), key=lambda i: similarities[i], reverse=True)
